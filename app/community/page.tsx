    ...
    <main className="min-h-screen bg-[#F4F3EE] pt-28 sm:pt-32 pb-20 px-4 sm:px-6 font-sans text-stone-900 selection:bg-stone-900 selection:text-[#F4F3EE]">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col items-center text-center mb-16">
          ...
          <p className="text-sm font-bold text-stone-700 max-w-xl mx-auto border-t-2 border-b-2 border-stone-900 py-4 border-dashed">
        ...
        {recentBooks.length === 0 ? (
          <div className="paper-card bg-[#FAFAFA] p-12 text-center border-dashed border-4 border-stone-300">
            <span className="text-sm font-bold text-stone-700 uppercase tracking-widest">Le tableau d'affichage est vide aujourd'hui.</span>
          </div>
        ) : (
              ...
              <div key={book.id} className="paper-card paper-hover bg-[#FAFAFA] p-4 flex flex-col relative group">
                ...
                <div className="w-full aspect-square mb-4 paper-card bg-stone-100 overflow-hidden relative border-2 border-stone-900">
                  {book.cover_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-[10px] font-black uppercase tracking-widest text-stone-500">Sans Image</div>
                  )}
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <p className="text-[10px] sm:text-xs font-bold text-stone-700 mb-4">{book.author}</p>
                  ...
                  {book.status === 'read' && book.notes && (
                    <div className="mt-auto bg-[#F4F3EE] p-3 border-l-4 border-stone-900 relative">
                      ...
                      <p className="text-xs font-serif italic text-stone-800 line-clamp-3 relative z-10">{book.notes}</p>
                  ...
                  <div className="mt-4 pt-3 border-t-2 border-stone-200 border-dashed flex justify-between items-center">
                     <span className="text-[8px] font-bold uppercase tracking-widest text-stone-500">Ajouté par le membre</span>
                     {/* BOUTON ENCRE - VOIR L'ETUI */}
                     <InkButton href={`/community/${book.user_id}`} isDark={true} className="px-3 py-1 text-[8px] font-black uppercase tracking-widest">
                       Voir son étui
                     </InkButton>
                  </div>
                </div>
