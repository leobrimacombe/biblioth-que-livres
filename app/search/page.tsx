...
export default function SearchPage() {
  ...
  const [maxResults, setMaxResults] = useState(10); // Le nouveau réglage !
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // On intègre la variable maxResults dans l'URL
      const url = apiKey 
  ...
  return (
    <main className="min-h-screen bg-zinc-50 pt-32 pb-20 px-6 font-sans text-zinc-900 transition-colors duration-500 overflow-x-hidden">
      <div className="max-w-5xl mx-auto">
        
        <div className="flex flex-col items-center text-center mb-12">
          <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.4em] text-zinc-400 mb-4">Explorateur</span>
          <h1 className="text-4xl md:text-6xl font-black tracking-tight text-zinc-900 mb-4">
            Rechercher un livre.
          </h1>
        </div>

        {/* Barre de recherche avec réglages */}
        <div className="max-w-2xl mx-auto mb-16">
          ...
          <div className="flex justify-end items-center gap-3 px-4">
            <label className="text-xs font-bold uppercase tracking-widest text-zinc-400">Afficher :</label>
            <select 
              value={maxResults}
              onChange={(e) => setMaxResults(Number(e.target.value))}
              className="bg-transparent text-sm font-bold text-zinc-900 outline-none cursor-pointer hover:text-zinc-600 transition-colors"
            >
              <option value={10}>10 résultats</option>
              <option value={20}>20 résultats</option>
              <option value={30}>30 résultats</option>
              <option value={40}>40 résultats (Max)</option>
            </select>
          </div>
        </div>
      ...
      {selectedBook && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm p-4 transition-all">
          ...
          <div className="relative w-full max-w-3xl bg-white rounded-3xl md:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden h-[85dvh] md:h-auto md:max-h-[85vh] animate-in zoom-in-95 duration-300">
            <button onClick={() => setSelectedBook(null)} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white/80 backdrop-blur-md text-zinc-900 shadow-md hover:bg-zinc-100 z-50 transition-colors border border-zinc-200">
            ...
            <div className="flex-1 overflow-y-auto flex flex-col md:flex-row min-h-0 relative">
              <div className="w-full md:w-2/5 bg-zinc-50 p-6 md:p-8 flex items-center justify-center border-b md:border-b-0 md:border-r border-zinc-100 shrink-0">
                {selectedBook.volumeInfo.imageLinks?.thumbnail ? (
                  <img src={selectedBook.volumeInfo.imageLinks.thumbnail.replace('&edge=curl', '')} alt="Couverture" className="w-auto h-48 md:h-auto md:max-w-[200px] object-contain rounded-xl shadow-[0_20px_40px_rgb(0,0,0,0.15)]" />
                ) : (
              ...
              <div className="w-full md:w-3/5 p-6 md:p-8 flex flex-col">
                ...
                <div className="text-sm leading-relaxed text-zinc-600 pb-4">
                  {selectedBook.volumeInfo.description ? selectedBook.volumeInfo.description : <span className="italic text-zinc-400">Aucun synopsis n'a été fourni pour cette édition.</span>}
                </div>
              </div>
            </div>
            <div className="shrink-0 bg-white p-4 md:p-6 border-t border-zinc-100 flex flex-col sm:flex-row gap-3 z-10 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
              <button onClick={() => handleSaveBook(selectedBook)} className="w-full sm:flex-1 rounded-full bg-zinc-900 px-4 py-4 md:py-3.5 text-xs font-bold uppercase tracking-widest text-white transition-all hover:bg-zinc-800 active:scale-95">Ajouter à la collection</button>
              <a href={`https://www.amazon.fr/s?k=${encodeURIComponent(selectedBook.volumeInfo.title + " livre " + (selectedBook.volumeInfo.authors?.[0] || ""))}`} target="_blank" rel="noopener noreferrer" className="w-full sm:flex-1 rounded-full border border-zinc-200 bg-white px-4 py-4 md:py-3.5 text-xs font-bold uppercase tracking-widest text-zinc-900 transition-all hover:bg-zinc-50 active:scale-95 text-center flex items-center justify-center">Voir l'édition</a>
            </div>
          </div>
        </div>
      )}
